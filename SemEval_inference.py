...
class PSPDetector(pl.LightningModule):
    ...
    def test_step(self, val_batch, batch_idx):
        ...
        FP = 0
        FN = 0
        f = open('/new_temp/fsb/fsb_PolitiStance/inference/test_fold_' + str(test_fold) + '.txt', 'w')
        for graph in val_batch:
            semantic_feature = self.relu(self.linear_before_RGCN_semantic(graph['roberta_feature']))
            try:
                entity_feature = self.dropout_layer(self.relu(self.linear_before_RGCN_entity(graph['entity_feature'])))
            ...
            avg_loss += loss
            if torch.argmax(prob) == graph['ground_truth']:
                f.write(str(len(graph['entity_feature'])) + ' ' + str(sum(graph['edge_type'] == 2).cpu().item()) + ' Y\n')
            else:
                f.write(str(len(graph['entity_feature'])) + ' ' + str(sum(graph['edge_type'] == 2).cpu().item()) + ' N\n')
...
checkpoint_path = 'lightning_logs/version_632/'
file = os.listdir(checkpoint_path + 'checkpoints/')[0]
print(file)
model = model.load_from_checkpoint(checkpoint_path = checkpoint_path + 'checkpoints/' + file, hparams_file = checkpoint_path + 'hparams.yaml', map_location = None, semantic_in_channels=768, entity_in_channels=200, out_channels=512, dropout=0.5, num_relations = 3)
