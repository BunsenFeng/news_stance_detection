...
class PSPDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        length = len(temp['roberta_feature']) - 1
        temp['edge_index'] = torch.cat((temp['edge_index'][:,:length], temp['edge_index'][:,2*length-1:]), dim = 1)
        temp['edge_type'] = torch.cat((temp['edge_type'][:length], temp['edge_type'][2*length-1:]), dim = 0)
        return temp
