...
from torch_geometric.nn import MessagePassing, RGCNConv, GATConv, GCNConv, SAGEConv
...
class PSPDetector(pl.LightningModule):
    def __init__(self, semantic_in_channels, entity_in_channels, out_channels, dropout, num_relations):
        ...
        self.RGCN = RGCNConv(self.out_channels, self.out_channels, self.num_relations)
        #self.GCN = GCNConv(self.out_channels, self.out_channels) # no edge_type for code down there!!
        #self.GAT = GATConv(self.out_channels, self.out_channels) # no edge_type for code down there!!
        #self.SAGE = SAGEConv(self.out_channels, self.out_channels) # no edge_type for code down there!!
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        for graph in train_batch:
            ...
            #node_feature = self.dropout_layer(self.relu(self.GatedRGCN(node_feature, graph['edge_index'], graph['edge_type'])))
            node_feature = self.dropout_layer(self.relu(self.RGCN(node_feature, graph['edge_index'], graph['edge_type'])))
            node_feature = self.dropout_layer(self.relu(self.RGCN(node_feature, graph['edge_index'], graph['edge_type'])))
            #node_feature = self.dropout_layer(self.relu(self.GCN(node_feature, graph['edge_index'])))
            #node_feature = self.dropout_layer(self.relu(self.GCN(node_feature, graph['edge_index'])))
            #node_feature = self.dropout_layer(self.relu(self.GAT(node_feature, graph['edge_index'])))
            #node_feature = self.dropout_layer(self.relu(self.GAT(node_feature, graph['edge_index'])))
            #node_feature = self.dropout_layer(self.relu(self.SAGE(node_feature, graph['edge_index'])))
            #node_feature = self.dropout_layer(self.relu(self.SAGE(node_feature, graph['edge_index'])))
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        for graph in val_batch:
            ...
            #node_feature = self.relu(self.GatedRGCN(node_feature, graph['edge_index'], graph['edge_type']))
            node_feature = self.relu(self.RGCN(node_feature, graph['edge_index'], graph['edge_type']))
            node_feature = self.relu(self.RGCN(node_feature, graph['edge_index'], graph['edge_type']))
            #node_feature = self.relu(self.GCN(node_feature, graph['edge_index']))
            #node_feature = self.relu(self.GCN(node_feature, graph['edge_index']))
            #node_feature = self.relu(self.GAT(node_feature, graph['edge_index']))
            #node_feature = self.relu(self.GAT(node_feature, graph['edge_index']))
            #node_feature = self.relu(self.SAGE(node_feature, graph['edge_index']))
            #node_feature = self.relu(self.SAGE(node_feature, graph['edge_index']))
            prob = self.linear_classification(torch.mean(node_feature[:len(graph['roberta_feature'])], dim = 0))
            loss = self.CELoss(prob.unsqueeze(0), torch.tensor(graph['ground_truth']).unsqueeze(0).long().cuda())
            avg_loss += loss
