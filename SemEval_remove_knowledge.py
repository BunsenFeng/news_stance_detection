...
class PSPDataset(Dataset):
    def __init__(self,  name, test_fold, remove_knowledge_percent):  # name = train/dev
        path = '/new_temp/fsb/fsb_PolitiStance/'

        self.name = name
        self.idlist = []
        self.remove_knowledge_percent = remove_knowledge_percent
    ...
    def __getitem__(self, index):
        ...
        fro = temp['edge_index'][0]
        to = temp['edge_index'][1]
        edge_type = temp['edge_type']
        id = list(np.random.permutation(len(edge_type))[int(self.remove_knowledge_percent / 100 * len(edge_type)):]) 
        temp['edge_index'] = torch.stack([fro[id], to[id]])
        temp['edge_type'] = temp['edge_type'][id]
        return temp
...
remove_knowledge_percent = int(input('percent of knowledge to remove: '))
#test_fold = 0
#remove_knowledge_percent = 30

# data
dataset1 = PSPDataset(name='train', test_fold = test_fold, remove_knowledge_percent = remove_knowledge_percent)
test_count = 0
...
dataset2 = PSPDataset(name='test', test_fold = test_fold, remove_knowledge_percent = remove_knowledge_percent)
