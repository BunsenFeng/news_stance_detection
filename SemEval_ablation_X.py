...
class PSPDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # remove RoBERTa
        temp['roberta_feature'] = torch.randn(len(temp['roberta_feature']), len(temp['roberta_feature'][0]))
        #remove TransE
        try:
            temp['entity_feature'] = torch.randn(len(temp['entity_feature']), len(temp['entity_feature'][0]))
        except:
            temp['entity_feature'] = temp['entity_feature']
        return temp
